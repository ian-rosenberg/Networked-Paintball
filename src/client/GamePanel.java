...
public class GamePanel extends BaseGamePanel implements Event {
    ...
    @Override
    public void onSetId(int id, String clientName) {
	System.out.println("Checking " + id + " " + clientName);
	for (Player player : players) {
	    if (player.getName() == null) {
		System.out.println("Player name null for " + id + " " + clientName);
	    }
	    if (player.getName() != null && player.getName().equalsIgnoreCase(clientName)) {
		player.setId(id);
		System.out.println("Set id for matched player");
		break;
	    }
	}
    }
	...
	if (idHP.x == -1) {
	    ...
	    for (Player player : players) {
		System.out.println(player.getId() + " vs " + idHP.x);
